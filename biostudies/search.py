...
class BioStudiesExtractor:
    ...
    def __init__(self, collection:str=""):
        ...
        self.search_url = f"{self.base_url}/{collection}/search" if collection else f"{self.base_url}/search"
    ...
    def validate_study_id(self, study_id):
        ...
        if not any(re.match(pattern, cleaned_id) for pattern in patterns):
            return False, cleaned_id, "Invalid BioStudies ID format. Expected format: S-ONTX26, E-MTAB-1234, etc."
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    if not data:
                        return {"error": f"Empty response received for study {cleaned_id}"}
                    return self.parse_metadata(data)
                except json.JSONDecodeError as e:
                    return {"error": f"Invalid JSON response from BioStudies API: {str(e)}"}
            elif response.status_code == 404:
                return {"error": f"Study '{cleaned_id}' not found in BioStudies database. Please check the ID and try again."}
        ...
        try:
            ...
            if "attributes" in raw_data:
                for attr in raw_data["attributes"]:
                    ...
                    if attr_name in ["organism", "species", "organism part", "organ", "cell type", "tissue", "disease", "disease state", "sample type"]:
